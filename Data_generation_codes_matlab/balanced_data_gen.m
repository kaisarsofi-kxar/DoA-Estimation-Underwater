...
% Angles, SNR, range, sensor depth
Angles = -90:1:89; % 180 angles
SNR = -6:2:10;
range = 1000:1000:5000;
sensor_depth = -20:-10:-100;

% Calculate the number of samples per combination
NumAngles = length(Angles); % 180
NumAnglePairs = nchoosek(NumAngles, sources); % 180 choose 2 = 16110
NumCombinations = length(sensor_depth) * length(range) * length(SNR); % 9 * 5 * 9 = 405
SamplesPerCombination = ceil(total_samples / NumCombinations); % ceil(150000 / 405) ≈ 371

% Adjust total samples to match exactly
total_samples = SamplesPerCombination * NumCombinations;

% Pre-generate all angle pairs
[angleIdx1, angleIdx2] = meshgrid(1:NumAngles, 1:NumAngles);
anglePairs = [angleIdx1(:), angleIdx2(:)];
anglePairs = anglePairs(anglePairs(:,1) < anglePairs(:,2), :); % Remove duplicate pairs
...
for pp = 1:length(sensor_depth)
    for jj = 1:length(range)
        for k = 1:length(SNR)
            Y_training = zeros(SamplesPerCombination, numberofSensors, numberofSensors);
            Y_label = zeros(SamplesPerCombination, numberofSensors, numberofSensors);
            Binary_DOAs1 = zeros(SamplesPerCombination, NumAngles);

            for i = 1:SamplesPerCombination
                disp(sam_num);

                % Select angle pairs systematically
                pairIdx = mod(sam_num-1, NumAnglePairs) + 1;
                randomThetas = Angles(anglePairs(pairIdx, :));
                binary_vector = zeros(1, NumAngles);
                disp(randomThetas)
                sprintf('range: %d, snr: %d, depth: %d',range(jj), SNR(k), sensor_depth(pp))
                ...
                [pos_tx1, vel_tx1] = beaconPlatform1(duration);
                [pos_rx, vel_rx] = arrayPlatform1(duration);
                [pos_tx2, vel_tx2] = beaconPlatform2(duration);
...
% Filter to ensure exactly 150,000 samples
if size(DOA_Labels, 1) > total_samples
    indices = randperm(size(DOA_Labels, 1), total_samples);
    Input_matrix = Input_matrix(indices, :, :);
    Label_matrix = Label_matrix(indices, :, :);
    DOA_Labels = DOA_Labels(indices, :);
end

save('balanced_dataset2.mat', 'Input_matrix', 'Label_matrix', 'DOA_Labels', '-v7.3');
