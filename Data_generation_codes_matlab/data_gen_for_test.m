%% Intilization
clear all; close all; clc; propSpeed = 1520; channelDepth = 120; OperatingFrequency = 4000; numberofSensors = 8; sources = 2;
...
for pp = 1:length(sensor_depth)
    for jj =1:length(range)
        for k=1:length(SNR)
            Y_training = zeros (Samples,fs,numberofSensors);
            Y_label = zeros (Samples,fs,numberofSensors);
            ...
            for i = 1:Samples
                ...
                noise = noise';
                Y_training(i,:,:)  = combined_signal + noise ;
                ...
                Y_label(i,:,:) = recieved_signal1 + recieved_signal2;
...
save('data_100_samples.mat', 'Input_matrix', 'Label_matrix', 'DOA_Labels','-v7.3');